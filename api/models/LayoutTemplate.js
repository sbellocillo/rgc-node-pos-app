...
class LayoutTemplate {
    constructor(data) {
        ...
        this.default_item_id = data.default_item_id;
    ...
    static async getAll(layout_id = null) {
        try {
            let query = `
                ...
                LEFT JOIN items i ON lt.default_item_id = i.id
    ...
    static async bulkUpsert(items) {
        ...
        try {
            ...
            for (const itemData of items) {
                // Validate individual item logic if needed
                if (!itemData.layout_id || !itemData.layout_indices_id) continue;

                const query = `
                    INSERT INTO layout_templates (layout_id, layout_indices_id, default_item_id)
                    VALUES ($1, $2, $3)
                    ON CONFLICT (layout_id, layout_indices_id)
                    DO UPDATE SET
                        default_item_id = EXCLUDED.default_item_id,
                        updated_at = CURRENT_TIMESTAMP
                    RETURNING *
                `;
                const values = [itemData.layout_id, itemData.layout_indices_id, itemData.default_item_id];
